const song = {
  ...
  getters: {
    ...
    rankingSongs: state => {
      let rankingSongs = state.rankingSongs;
      if (!listOfSongs.length) {
        listOfSongs = JSON.parse(window.sessionStorage.getItem('rankingSongs') || null);
      }
      return rankingSongs;
    },
    isPlay: state => {
  ...
  mutations: {
    ...
    setrankingSongs: (state, rankingSongs) => {
      state.rankingSongs = rankingSongs;
      window.sessionStorage.setItem('rankingSongs', JSON.stringify(rankingSongs));
    },
